...
app.patch("/api/manga/:manga_id", async (req, res) => {
  try {
    ...
    const mangaFields = [
      'title', 'description', 'cover_art_url', 
      'status', 'latest_chapter', 'latest_chapter_date'
    ];
    ...
    if (updates.genres !== undefined) {
      ...
      await db.query(
        'DELETE FROM mangagenres WHERE manga_id = $1',
        [manga_id]
      );
...
app.get('/api/manga', async (req, res) => {
  try {
    ...
    const sortOptions = {
      ...
      'unread': { 
        field: '(COALESCE(m.latest_chapter, 0) - COALESCE(w.last_chapter_read, 0))',
        order: 'DESC' 
      }
    };
